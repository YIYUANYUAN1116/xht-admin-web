...
const setPermisstion = async (row: RoleData) => {
  ...
  if (result.code == 200) {
    menuArr.value = result.data.menuArr;
...
const filterSelectArr = (allData: any, initArr: any) => {
  allData.forEach((item: any) => {
    if (item.selected == 1 && item.level == 4) {
...
const handler = async () => {
  ...
  //下发权限
  let roleMenu: RoleMenuParams = { roleId, permissionIds };
  console.log(roleMenu);
  let result: any = await reqSetPermisstion(roleMenu);
  if (result.code == 200) {
    //抽屉关闭
    drawer.value = false;
    //提示信息
