          ...
          <el-table-column label="销售属性值">
            ...
            <template #="{ row, $index }">
              <el-tag
                ...
                @close="row.attrValueList.splice(index, 1)"
...
const toLook = (row: SaleAttr) => {
  ...
  let newSaleAttrValue: SaleAttrValue = {
    baseSaleAttrId,
    valueName: saleAttrValue as string,
  };
  ...
  //判断属性值是否在数组当中存在
  let repeat = row.attrValueList.find((item) => {
    return item.valueName == saleAttrValue;
  });
  ...
  //追加新的属性值对象
  row.attrValueList.push(newSaleAttrValue);
  //切换为查看模式
  row.flag = false;
};
...
const handleRemove = async (
  ...
  let img: SpuImg = { id: uploadFile.id, imgUrl: uploadFile.url };

  let result = await deleteSpuImage(img);
...
const initAddSpu = async (c3Id: number | string) => {
  ...
  //获取全部品牌的数据
  let result: AllTradeMark = await reqAllTradeMark();
  let result1: HasSaleAttrResponseData = await reqAllSaleAttr(c3Id);
  //存储数据
  AllTradeMark.value = result.data;
